...
export class RentalService {
  ...
  getRentalDetailById(id: number): Observable<RentalDetail> {
    return this.http.get<RentalDetail>(`${this.baseUrl}/${id}`).pipe(
  ...
  undoRentalDetailUpdate(rental: RentalDetail): Observable<RentalDetail> {
    ...
    // If it means to reset the data to some previous state, you'll need to have stored that state somewhere.
    // Here, we'll assume you want to re-fetch the original data from the server.
    return this.getRentalDetailById(rental.lotNumber);
  }
}
